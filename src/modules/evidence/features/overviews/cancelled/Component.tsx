import { Description, RestoreFromTrash } from "@material-ui/icons";
...
import { recoverDialogOpen } from "share/components/dialog/recoverDialog/_actions";
...
const getColumns = (session: SessionType): DataColumn<GenericDocument>[] => {
  const columns: DataColumn<GenericDocument>[] = [
    ...
    {
      keys: [
        classPath(genericDocumentProxy.properties!.ssl!.cancelReason).path
      ],
      ...
      items: [
        {
          action: (selected: GenericDocument[]) =>
            dispatch(recoverDialogOpen(selected)),
          icon: <RestoreFromTrash />,
          title: t(translationPath(lang.general.restore))
        }
      ]
    },
    single: {
  ...
  return (
    <MenuLayout>
      <DocumentView
        ...
        controls={controls}
        customTitle={t(translationPath(lang.table.canceled))}
        defaultSortAsc={true}
